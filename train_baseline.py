...
def main(config):
    ...
    for run_number in range(int(config['settings']['repeated_runs'])):
        ...
        CHECKPOINT = f"./{config['settings']['model_dir']}/{config['settings']['experiment_name']}_{config['model']['model_architecture']}_run_{run_number}.pth"
    ...
    df.to_csv(f"./logs/{config['settings']['experiment_name']}_{config['model']['model_architecture']}_metrics.csv")
